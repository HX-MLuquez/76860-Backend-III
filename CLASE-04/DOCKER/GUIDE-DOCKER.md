...
# PASOS para crear un contenedor Docker desde cero

1. Instalar Docker
2. Crear un proyecto (por ejemplo, una app Node.js)
3. Crear un archivo `Dockerfile` en el raíz del proyecto
4. Detallar las instrucciones para construir la imagen
...
```bash
docker build -t zenon-image-v1 .
```

6. Crear y correr el contenedor

```bash
docker run -d -p 8080:3000 --name zenon-container zenon-image-v1
```

## Pasos para crear una imagen y subirla a Docker Hub
...
```bash
docker build -t mauricio776/zenon-image:1.0.0 .
```

3. Subir la imagen

```bash
docker push mauricio776/zenon-image:1.0.0
```

4. Descargar la imagen desde Docker Hub

```bash
docker pull mauricio776/zenon-image:1.0.0
```
